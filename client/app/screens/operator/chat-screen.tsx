 ...
 * 
 * Detailed chat interface between operator and client.
 */
...
export default function OperatorChatScreen() {
    ...
    const handleSend = async () => {
        ...
        if (!request) {
            Alert.alert('Error', 'No active job found. You cannot send messages without an active job.');
            return;
        }
            ...
            <SafeAreaView edges={['top', 'bottom']} style={styles.safeArea}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.headerContent}>
                        <ThemedText style={styles.headerTitle}>{request?.user?.fullName || 'Client'}</ThemedText>
                        <ThemedText style={styles.headerSubtitle}>Active Job</ThemedText>
                    </View>

                    <TouchableOpacity
                        style={[styles.callButton, { backgroundColor: '#dcfce7' }]}
                        onPress={() => request?.user?.phone && Linking.openURL(`tel:${request.user.phone}`)}
                    >
